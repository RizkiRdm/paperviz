...
- **Evidence provenance (Chunk 1.1):** `evidence` table (migration 005) with `paper_id` FK cascade, `page`, `figure_id`, `table_id`, `section`, `source_text`, `source_reference`. `EvidenceRepo` (Insert/ListByPaper). Pipeline populates one row per image-origin chart (`savePipelineResult`, from `Services.Chart.SourceText` set in `reVisualizeOne` from page text); exposed in `GET /api/documents/:id` as `evidence[]`. Chapter-derived charts produce no rows (no original page mapping).
...
- **Chart image serving (Chunk 1.2):** `GET /api/documents/:id/charts/:chartId/image` serves original figure bytes (MIME-sniffed PNG/JPEG/GIF/WebP; lookup scoped to parent document). GET chart response carries `image_url` for `image_fallback` charts. `ChartCard` renders two labeled zones — Original Figure (image + page badge) vs PaperViz AI Interpretation (Recharts chart or annotation).
- **Paper History titles (Chunk 2.1):** Migration 006 adds `title TEXT NOT NULL DEFAULT ''` to documents (backfilled from first 200 chars of original_text). `deriveTitle()` in intake.go extracts first non-empty line, capped at 200 chars, fallback "Untitled paper". `DocumentListItem` struct carries lightweight list rows. `ListSummariesByUser` uses correlated subqueries for chart_count, explanation_count, summary_preview. Dashboard rows show title/date/status/summary/counts. `title` added to Document struct and GET response.
