...
- **All three providers route through goai.** `github.com/zendev-sh/goai` owns provider protocol for `gemini`, `anthropic`, and `openai`. Everything around it stays ours: `Transport` owns the retry schedule, the 90s budget, and the single process-wide concurrency slot, and the SDK's own retry is disabled with `WithMaxRetries(0)` so the two cannot compound. Adding a fourth provider means a `newGoAIModel` case, not a new client.
